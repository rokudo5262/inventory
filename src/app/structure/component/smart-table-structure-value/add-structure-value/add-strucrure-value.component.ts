...
import { StructureValueApiActions, StructureApiActions } from '@app/structure/actions';
...
export class StructureValueAddComponent implements OnInit {
  ...
  submit(item) {
    if (item.companyCode !== '' &&
      ...
      // }
      // else{
      this.store1.dispatch(StructureValueApiActions.addStructureValue({ structurevalue: item }));
    }
    this.close();
  }
}
