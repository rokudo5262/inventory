...
@Component({
  selector: 'ngx-structure-value-edit-dialog',
  templateUrl: './edit-structure-value.component.html',
  styleUrls: ['./edit-structure-value.component.scss']
})
export class StructureValueEditDialogComponent implements OnInit {
  ...
  constructor(
    ...
    protected dialogRef: NbDialogRef<StructureValueEditDialogComponent>,
  ...
  submit(item) {
    if (item.companyCode !== '' &&
      ...
      if (item.id !== 0) {
        ...
        this.store.dispatch(StructureValueApiActions.updateStructureValue({ update: update }));
      }
      // else{
      //   this.store.dispatch(StructureValueApiActions.addStructureValue({structurevalue: item}))
      // }
    }
    this.close();
  }
